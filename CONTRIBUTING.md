...
```bash
git clone https://github.com/USERNAME/dio-lab-open-source.git
```
Pressione enter, e uma cópia do seu fork no GitHub será criada localmente.

### 3) Crie uma nova **branch** 
Utilize o comando `git checkout -b` para criar e alternar para a nova branch e nomeie-a como `feat/community/USERNAME`
...
Utilize o comando `git add community/USERNAME.md` para adicionar sua alteração (nesse caso o arquivo markdown criado)  à "staging area" no Git.

### 6) Crie um Commit
Crie um commit e adicione a mensagem indicando a adição do seu perfil:
```bash
git commit -m"feat: add USERNAME profile"
```
>[!IMPORTANT]
...
- Seu PR deve modificar apenas o arquivo community/USERNAME.md (dê uma olhadinha na aba "Files changed");
